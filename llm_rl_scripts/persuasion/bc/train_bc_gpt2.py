...
def main(
    ...
    def str_iterable(items: List[str]):
        for item in items:
            for i in range(0, len(item['conversation']) - 1, 2):
                in_text = " ".join(item['conversation'][:i+1])
                out_text = item['conversation'][i+1]
    ...
    def evaluator(inference: GPT2Inference):
        ...
        generation_examples = eval_items
        if len(generation_examples) >= generation_bsize*generation_batches:
            generation_examples = generation_examples[:generation_bsize*generation_batches]
        ...
        generation_data = generate_language(
            ...
            references=list(map(lambda x: [x['out_text']], generation_examples)),
            prng_key=new_prng, 
