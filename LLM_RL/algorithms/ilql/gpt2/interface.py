...
<<<<<<< Updated upstream
                ...
                masked_idxs = (
                    ...
                    jnp.flip(should_take_action, axis=1).astype(jnp.int32) * (should_take_action.shape[1] + 1)
                )
                # print(masked_idxs.shape)
                next_action_idxs = jax.lax.cummin(masked_idxs[:, ::-1], axis=1)[:, ::-1]
                # print(next_action_idxs.shape)
                last_action_idxs = (should_take_action.shape[1]-1)-jnp.argmax(jnp.flip(should_take_action, axis=1).astype(jnp.int32), axis=1)+1
                # print(last_action_idxs.shape)
                next_action_idxs = jnp.minimum(next_action_idxs.shape[0], last_action_idxs[..., None] * jnp.ones_like(next_action_idxs))
                # print(next_action_idxs.shape)
                
                v = jnp.take_along_axis(v, next_action_idxs, axis=1)
                v = jnp.concatenate([v_full[:, :1], v], axis=1)
                # print(v.shape)
                # embed()
...
<<<<<<< Updated upstream
                    v_final = v[jnp.arange(0, should_take_action.shape[0], dtype=jnp.int32), final_state_idxs]
                    v_final = v_final * (1 - dones)
                    
                v_final = jax.lax.stop_gradient(v_final)
                assert v_final.shape == (128,)
=======
