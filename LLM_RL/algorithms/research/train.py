...
from LLM_RL.algorithms.research.base_interface import Train, Inference
...
def dump_state(
    ...
    **loop_state: Dict[Hashable, Any], 
    ...
    # save model
    if enable_save:
        create_path(os.path.join(save_dir, 'model'))
    # dump base_model config
    with open(get_enabled_save_path(os.path.join(save_dir, 'model', 'config.json'), enabled=enable_save), 'w') as f:
        f.write(model.config.to_json_string())
    # dump train_state
    if save_train_state:
        save_pytree(
            tree=train_state, 
            path=get_enabled_save_path(os.path.join(save_dir, 'model', 'train_state.msgpack'), enabled=enable_save), 
            dtype=save_dtype, 
        ...
        save_pytree(
            ...
            path=get_enabled_save_path(os.path.join(save_dir, 'model', 'params.msgpack'), enabled=enable_save), 
            dtype=save_dtype, 
    ...
    # save target_base
    if enable_save:
        create_path(os.path.join(save_dir, 'target'))
    if target_base_params is not None:
        # dump target_base_model config
        with open(get_enabled_save_path(os.path.join(save_dir, 'target', 'config.json'), enabled=enable_save), 'w') as f:
            f.write(model.config.to_json_string())
        # dump params
        save_pytree(
            tree=target_params, 
            path=get_enabled_save_path(os.path.join(save_dir, 'target', 'params.msgpack'), enabled=enable_save), 
            dtype=save_dtype, 
            sharding=get_sharding_from_model(model, target_params), 
        )

    # save pi_beta
    if enable_save:
        create_path(os.path.join(save_dir, 'pi_beta'))
    # dump base_model config
    with open(get_enabled_save_path(os.path.join(save_dir, 'pi_beta', 'config.json'), enabled=enable_save), 'w') as f:
        f.write(model.config.to_json_string())
    # dump train_state
    if save_train_state:
        save_pytree(
            tree=pi_beta_train_state, 
            path=get_enabled_save_path(os.path.join(save_dir, 'pi_beta', 'train_state.msgpack'), enabled=enable_save), 
            dtype=save_dtype, 
            sharding=get_sharding_from_model(model, pi_beta_train_state), 
        )
    else:
        save_pytree(
            tree=pi_beta_train_state.params, 
            path=get_enabled_save_path(os.path.join(save_dir, 'pi_beta', 'params.msgpack'), enabled=enable_save), 
            dtype=save_dtype, 
            sharding=get_sharding_from_model(model, pi_beta_train_state.params), 
        )
    ...
    **loop_state: Dict[Hashable, Any], 
    ...
    def _save(
        ...
        **loop_state: Dict[Hashable, Any], 
        ...
        dump_state(
            ...
            target_params=trainer.target_params,
            pi_beta_train_state=trainer.pi_beta_train_state,
    ...
    def _inference_update():
        ...
        inference = inference.replace(
            ...
            target_params=trainer.target_params,
            pi_beta_params=trainer.pi_beta_train_state.params,
        )
