    ...
    def load_train(
        ...
        def _step(
            ...
            def grad_loss(base_params: PyTree, q1_head_params: PyTree, q2_head_params: PyTree, v_head_params: PyTree, prng_key: jax.random.PRNGKeyArray):
                ...
                q1 = jnp.take_along_axis(jax.nn.softmax(q1_head_output[:, :-1]), input_ids[:, 1:][..., None], axis=2).squeeze(2)
                q2 = jnp.take_along_axis(jax.nn.softmax(q2_head_output[:, :-1]), input_ids[:, 1:][..., None], axis=2).squeeze(2)
                v = v_head_output[:, :-1].squeeze(2)
                v_full = v_head_output.squeeze(2)
                target_q1 = jnp.take_along_axis(jax.nn.softmax(target_q1_head_output[:, :-1]), input_ids[:, 1:][..., None], axis=2).squeeze(2)
                target_q2 = jnp.take_along_axis(jax.nn.softmax(target_q2_head_output[:, :-1]), input_ids[:, 1:][..., None], axis=2).squeeze(2)
